...
ttk.Entry(app,textvariable=income).grid(row=1, column=1)

num_rooms=ttk.Variable(app)
ttk.Label(app, text='Number of Rooms', padx=15, pady=15).grid(row=2, column=0)
ttk.Entry(app,textvariable=income).grid(row=2, column=1)

Population=ttk.Variable(app)
ttk.Label(app, text='Population', padx=15, pady=15).grid(row=3, column=0)
ttk.Entry(app,textvariable=income).grid(row=3, column=1)

def prediction():
    global model 
    query_data = {'Avg. Area Income':[eval(income.get())], 'Avg. Area House Age':[eval(house-age.get())],'Avg. Area Number of Rooms':[eval(num_rooms.get())],'Area Population':[eval(population.get)]
}
